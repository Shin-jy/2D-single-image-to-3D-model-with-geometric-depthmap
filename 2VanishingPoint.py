...
def ransac_vanishing_point(edgelets, num_ransac_iter=2000, threshold_inlier=5):
    ...
    for ransac_iter in range(num_ransac_iter):
        ind1 = np.random.choice(first_index_space) #line1
        ind2 = np.random.choice(second_index_space)#line2

        l1 = lines[ind1]
        l2 = lines[ind2]

        current_model = np.cross(l1, l2) #cross point of line1 &line2 => vanishing point
    ...
    return best_model #vanishing point coordinates
...
def vis_model(image, model, show=True):
    ...
    plt.savefig("./output/vis_LineE.png" )
    ...
    if show:
        plt.show()
        plt.savefig("./output/vis_VanishingPoint.png" )
